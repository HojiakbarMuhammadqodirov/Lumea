...
export default function HomePage({ onNav, onStartTest }) {
  ...
  // First unfinished lesson across all courses
  let nextLesson = null;
  ...
  return (
    <>
            ...
            {nextLesson ? (
              <>
                ...
                <div>
                  ...
                  <div className="text-xs text-[#9EB3C8] mt-0.5">{nextLesson.courseLabel} · {nextLesson.dur} daqiqa</div>
                </div>
              </>
            ) : (
              <>
                ...
                <div className="text-sm text-[#9EB3C8]">Barcha darslar bajarildi!</div>
              </>
            )}
          </Card2>
      ...
      <Grid cols={2}>
        <Card>
          ...
          {tests.slice(0, 3).map((t, i) => (
            <div key={t.id} className={`flex items-center gap-2.5 px-3 py-2 rounded-xl mb-1.5 ${i === 0 ? "bg-[#F0F5FC] border border-[#DDE6F0]" : ""}`}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold shrink-0" style={{ background: t.type === "sat" ? "rgba(37,99,235,0.09)" : "rgba(15,158,106,0.09)", color: t.type === "sat" ? "#2563EB" : "#0F9E6A" }}>
                {t.score}
              </div>
              <div className="flex-1 min-w-0">
      ...
      <div className="bg-[#0F2746] rounded-2xl p-5 flex items-center justify-between gap-4">
        ...
        <button onClick={() => showToast("Pro rejim tez kunda!")} className="shrink-0 bg-white text-[#173B64] font-bold text-sm px-5 py-2.5 rounded-xl hover:bg-[#F0F5FC] transition-colors whitespace-nowrap">
          Pro ga o'tish
        </button>
      </div>
    </>
