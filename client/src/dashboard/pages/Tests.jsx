...
export default function TestsPage() {
  ...
  const startTest = () => { setTestOn(true); setQi(0); setSel(null); setAnswered(false); setScore(0); };

  const check = () => { if (sel === null) return; setAnswered(true); if (sel === questions[qi].ans) setScore(s => s + 1); };
  const next = () => {
    if (qi < questions.length - 1) { setQi(q => q + 1); setSel(null); setAnswered(false); }
    else { showToast(`Test tugadi! Ball: ${score + (sel === questions[qi].ans ? 1 : 0)}/${questions.length} 🎉`); setTestOn(false); }
  };
  ...
  if (testOn) {
    ...
    return (
      <Card className="max-w-2xl mx-auto">
        ...
        {answered && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-3.5 mt-2 text-sm text-green-800 leading-relaxed">
            {q.exp}
          </div>
        )}
  ...
  return (
    <>
      <Grid cols={3} gap={12}>
        ...
        <StatCard value={Math.max(...tests.map(t => t.score))} label="Eng yuqori ball" color="#0F9E6A" />
      ...
      <Card>
        ...
        <div className="flex flex-col gap-1.5">
          {tests.map((t, i) => (
            <div key={t.id}
              className={`flex items-center gap-3 px-3.5 py-3 rounded-xl ${i === 0 ? "bg-[#F0F5FC] border border-[#DDE6F0]" : "border border-transparent"}`}
            >
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-sm font-bold shrink-0
                ${t.type === "sat" ? "bg-blue-50 text-blue-600" : "bg-green-50 text-green-700"}`}>
                {t.score}
              </div>
