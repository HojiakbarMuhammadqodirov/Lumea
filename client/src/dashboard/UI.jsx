...
export function PBar({ pct, color, height = 5 }) {
  return (
    <div style={{ height }} className="bg-[#DDE6F0] rounded-full overflow-hidden">
      <div
        style={{ width: `${pct}%`, height, background: color, transition: "width 1s cubic-bezier(.4,0,.2,1)" }}
...
export function Card({ children, className = "", onClick }) {
  return (
    <div
      onClick={onClick}
      className={`bg-white border border-[#DDE6F0] rounded-2xl p-5 ${onClick ? "cursor-pointer hover:border-[#b5c8e0] transition-colors" : ""} ${className}`}
    >
      {children}
    </div>
  );
...
export function Card2({ children, className = "" }) {
  return (
    <div className={`bg-[#F0F5FC] border border-[#DDE6F0] rounded-xl p-4 ${className}`}>
      {children}
    </div>
  );
}
...
export function SLabel({ children, className = "" }) {
  return (
    <div className={`text-[10px] text-[#9EB3C8] uppercase tracking-widest font-bold mb-2.5 ${className}`}>
      {children}
    </div>
  );
}
...
export function Avatar({ initials, size = 32 }) {
  return (
    <div
      ...
      className="rounded-full bg-[#173B64] flex items-center justify-center text-white font-bold shrink-0"
    >
      {initials}
    </div>
  );
...
export function IconBox({ icon, color, glow, size = 34 }) {
  return (
    <div
      style={{ width: size, height: size, background: glow || "rgba(37,99,235,0.09)", color }}
      className="rounded-xl flex items-center justify-center text-base font-bold shrink-0"
    >
      {icon}
    </div>
  );
...
export function Row({ children, noBorder = false, className = "" }) {
  return (
    <div className={`flex items-center gap-3 py-3 ${!noBorder ? "border-b border-[#DDE6F0]" : ""} ${className}`}>
      {children}
    </div>
  );
}
    ...
    <div className="flex items-end gap-1.5 h-14">
      {data.map((v, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          <div
            style={{ height: `${Math.max((v / max) * 44, 4)}px`, background: i === todayIdx ? "#173B64" : "#DDE6F0" }}
            className="w-full rounded-sm transition-all"
          />
          <div className={`text-[9px] ${i === todayIdx ? "text-[#173B64] font-bold" : "text-[#9EB3C8]"}`}>
            {days[i]}
          </div>
        </div>
      ))}
...
export function QuestionOption({ label, text, state, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-start gap-3 border rounded-xl px-4 py-3 mb-2.5 transition-all ${styles[state] || styles.idle}`}
    >
  ...
  return (
    <div
      ...
      className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3 rounded-2xl shadow-lg text-sm font-medium
        ${isErr ? "bg-red-50 border border-red-200 text-red-700" : "bg-[#173B64] text-white"}`}
    >
      <span>{isErr ? "⚠️" : "✅"}</span>
      {toast.msg}
    </div>
